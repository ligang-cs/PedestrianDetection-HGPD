...
class RPNTestMixin(object):
    ...
    def simple_test_rpn(self, conv4, x, img_meta, rpn_test_cfg):
        rpn_outs = self.rpn_head(x)
        proposal_inputs = rpn_outs + (img_meta, rpn_test_cfg)
        proposal_list_stage1 = self.rpn_head.get_bboxes(
                        *proposal_inputs, is_stage1=True)
        rpn_out_stage2 = self.rpn_head.two_step_regress(conv4, x)
        proposal_inputs_stage2 = rpn_out_stage2 + (img_meta, rpn_test_cfg)
...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes(self,
        ...
        rois = bbox2roi(proposals)
        num_proposal_list = [proposal.size(0) for proposal in proposals]
        # roi_feats = self.bbox_roi_extractor(
        #     x[:len(self.bbox_roi_extractor.featmap_strides)], rois)\
        roi_feats = self.bbox_roi_extractor(
            conv4[:len(self.bbox_roi_extractor.featmap_strides)], rois)
        if self.with_shared_head:
            roi_feats = self.shared_head(roi_feats)
        cls_score, bbox_pred = self.bbox_head(
                                    roi_feats, rois[:, 1:], num_proposal_list)
        img_shape = img_meta[0]['img_shape']
        scale_factor = img_meta[0]['scale_factor']
        det_bboxes, det_labels = self.bbox_head.get_det_bboxes(
            rois,
