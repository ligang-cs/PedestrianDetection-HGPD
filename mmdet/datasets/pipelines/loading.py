...
class LoadImageFromFile(object):
    ...
    def __call__(self, results):
        if results['img_prefix'] is not None:
            if 'cityname' in results['img_info']:
                filename = osp.join(results['img_prefix'], results['img_info']['cityname'],
                                    results['img_info']['filename'])
            else:
                cityname = results['img_info']['filename'].split('_')[0]
                filename = osp.join(results['img_prefix'],cityname,
                                    results['img_info']['filename'])
        else:
...
class LoadAnnotations(object):
    ...
    def _load_masks(self, results):
        ...
        gt_masks_weight = results['ann_info']['masks_weight']
        if self.poly2mask:
